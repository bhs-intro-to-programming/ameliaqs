...
const moreThanTwiceAsLong = (x, y) => { (x > y) > 2 ? 'true' : 'false' }
const aFartherThanB = (a, b, c) => { Math.abs(a - c) > Math.abs(b - c) ? 'true' : 'false' }
...
const containsX = (s) => s.search([x]) >= 0 ? 'true' || 'false'
const sumSquares = (x) => {
  for (i = 0, i < x, i++) {
    x + Math.floor(x - i) ** 2
  }
